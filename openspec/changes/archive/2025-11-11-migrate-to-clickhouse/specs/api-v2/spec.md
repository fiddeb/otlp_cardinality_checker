# API v2: ClickHouse-Optimized Endpoints

> Capability: api-v2
> Status: PROPOSED
> Related Specs: clickhouse-storage, performance

## Overview

This spec defines new `/v2/` REST API endpoints that leverage ClickHouse's analytical capabilities. These endpoints provide enhanced cardinality analysis, pattern exploration, and performance metrics not available with the memory-only backend.
...
- None (all v1 endpoints remain unchanged)

## Cross-References

- **clickhouse-storage**: V2 endpoints require ClickHouse backend
- **api (existing spec)**: V1 endpoints defined in existing api spec, unchanged
- **performance**: Load tests will benchmark v2 endpoint latency vs v1

...
> Capability: clickhouse-storage
> Status: PROPOSED
> Related Specs: api-v2, performance

## Overview

This spec defines the ClickHouse storage backend implementation requirements. The backend uses ClickHouse's columnar storage and MergeTree engines for high-performance telemetry metadata storage.
...
- **api-v2**: API v2 endpoints will query ClickHouse directly
- **performance**: Load tests will benchmark ClickHouse vs Memory backend
- **storage (existing spec)**: ClickHouse replaces SQLite requirements, Memory requirements unchanged

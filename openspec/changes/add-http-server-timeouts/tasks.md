...
- [ ] **T-01** Add `ReadTimeout`, `WriteTimeout`, `IdleTimeout` to the `http.Server` literal in `internal/receiver/http.go`.
...
- [ ] **T-02** Add `http.MaxBytesReader(w, req.Body, 32<<20)` at the top of `handleMetrics`, `handleTraces`, and `handleLogs` in `internal/receiver/http.go`.
...
- [ ] **T-03** Add `ReadTimeout`, `WriteTimeout`, `IdleTimeout` to the `http.Server` literal in `internal/api/server.go`.
  - Same values as T-01.
  - Verify: `grep -n "ReadTimeout" internal/api/server.go` shows all three fields.

- [ ] **T-04** Write integration test for OTLP receiver timeout in `internal/receiver/http_timeout_test.go`.
...
- [ ] **T-05** Write integration test for API server timeout in `internal/api/server_timeout_test.go`.
  - Same approach as T-04 for the REST API server.

- [ ] **T-06** Run `go build ./...` — confirm no compilation errors.

- [ ] **T-07** Run `go test ./...` — confirm all tests pass, no regressions.

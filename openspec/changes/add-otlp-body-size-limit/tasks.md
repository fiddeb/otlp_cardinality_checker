...
- [x] **T-01** Confirm `http.MaxBytesReader` is present (from `add-http-server-timeouts`
...
- [x] **T-02** Add `Content-Length` pre-check at the top of `handleMetrics`,
...
- [x] **T-03** Write unit tests in `internal/receiver/http_body_limit_test.go`
...
- [x] **T-04** Run `go build ./...` — confirm no compilation errors.

- [x] **T-05** Run `go test ./internal/receiver/...` — confirm all new tests pass
